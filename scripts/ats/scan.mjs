...
const PORTALS_PATH = process.env.REALLOCATION_ENGINE_PORTALS || 'data/ats/portals.yml';
const SCAN_HISTORY_PATH = 'data/ats/scan-history.tsv';
const PIPELINE_PATH = 'data/ats/pipeline.md';
const APPLICATIONS_PATH = 'data/ats/applications.md';
const PROVIDERS_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'providers');

// Ensure required directories exist (fresh setup)
mkdirSync('data/ats', { recursive: true });
...
function appendToPipeline(offers) {
  ...
  if (!existsSync(PIPELINE_PATH)) {
    writeFileSync(PIPELINE_PATH, '# ATS Job Pipeline\n\n## Pending\n\n## Processed\n', 'utf-8');
  }
  ...
  if (idx === -1) {
    // No Pending section - append at end before Processed when present.
    const procIdx = text.includes('## Processed') ? text.indexOf('## Processed') : text.indexOf('## Procesadas');
...
async function main() {
  ...
  console.log(`\nReview new offers in ${PIPELINE_PATH}.`);
}
