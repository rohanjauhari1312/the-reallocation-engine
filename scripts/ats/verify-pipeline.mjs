...
const ATS_DATA_DIR = join(REPO_ROOT, 'data/ats');
const APPS_FILE = join(ATS_DATA_DIR, 'applications.md');
const ADDITIONS_DIR = join(ATS_DATA_DIR, 'tracker-additions');
const REPORTS_DIR = join(ATS_DATA_DIR, 'reports');

// Ensure required directories exist (fresh setup)
mkdirSync(ATS_DATA_DIR, { recursive: true });
...
for (const e of entries) {
  ...
  if (!match) continue;
  const reportPath = join(REPO_ROOT, match[1]);
