...
These skills are student-facing operating recipes for The Reallocation Engine.
They are not a substitute for the verified datasets and maintained scripts in
`scripts/`.

## Executive Summary

Every skill serves two customers. Agents read the skill to execute work:
required reads, commands, checks, outputs, stop conditions, and logging. Humans
read the skill to understand what the agent is about to do, why it is safe, and
where human judgment still matters.

Each skill should begin with a short human-readable summary, then provide
...
Do not turn a skill into "ask the model what it thinks" when a script, audit, or
...
| Run log | `skills/RUN_LOG.md` | Human-readable history of what was run, what worked, and what failed. |
...
6. **Logging gate:** Meaningful changes, blockers, and generated artifacts are
   recorded in `skills/RUN_LOG.md`.

## Logging Rules

Update `skills/RUN_LOG.md` whenever a skill:
...
- **Skill:** scan | pipeline | oferta | pdf | tracker | research | manual
...
These skills have been converted from legacy prompt recipes into verified
Reallocation Engine workflows. If a future skill references unsupported files or
