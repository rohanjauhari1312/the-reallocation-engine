# Skill: auto-pipeline -- Not Yet Automated

## Executive Summary

This skill documents why the full pipeline is not yet automated. Agents should
...
8. **Logging gate:** Runs produce `skills/RUN_LOG.md` and audit artifacts.

## Use This Sequence Instead

1. `skills/scan.md` -- detect ATS/provider/job URLs.
2. `skills/pipeline.md` -- verify URL liveness and move URLs through triage.
3. `skills/oferta.md` -- evaluate one role using company, sponsorship, ATS,
   liveness, BLS/SOC, and CV evidence.
4. `skills/pdf.md` -- generate an ATS-friendly PDF from an approved Markdown CV.
5. `skills/tracker.md` -- record status changes and outcomes.
...
- a log entry in `skills/RUN_LOG.md`.

Until that script exists, do not describe this skill as automatic.
